...
d3ej <-
    subset(ddde, (hour > 17 |
              hour <= 9) & !is.na(ddde$date))  # ddde de jour
...
d3ej$ex[d3ej$position == "n"] <-
    4 # le 2 est effacé par les pb techniques
...
Dj$day[Dj$day<250 & !leap_year] <- Dj$day[Dj$day<250 & !leap_year]+365
Dj$day[Dj$day<250 & leap_year] <- Dj$day[Dj$day<250  & leap_year]+365
...
sum(duplicated(paste(Dj$ndays,Dj$dsf_season)))
...
Dj[Dj$ndays%in%duplicated_ndays,]
...
#d3edj[p1,"N.2"]
D7j[p1,"N.2"] <- 0
D7j[p1,"N.2"] <- predict(g10,newdata=D7j[p1,],type="response")
...
<<positionnement_deprecated, echo=FALSE, eval=TRUE,results=hide>>=
...
précédente mais les débits à 3 jours (delta 3d) ont été utilisés à la place des
débits à 7 jours.
