...
\bigskip
{ \LARGE en 2021--2022 (10ème année)}
...
Les dévalaisons d'anguilles argentées ont été suivies, pour la dixième année
 ...
 c'est à dire quand le didson est bien positionné par rapport aux écoulements,
  ...
  \smallskip
 Silver eel migrations were monitored, for the tenth year
...
briand_suivi_2021, briand_suivi_2022}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{2022/debits_ajustes2022}
\caption[Débits recalculés 2022]{Débits recalculés en \Sexpr{CY}.
...
L'analyse des données de débit des volets montre que leur fonctionnement a été
continu en 2021-2022 (Figure \ref{debitvoletjour}).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{2022/debitvolet2022}
\caption[Débits volet 2022]{Débits recalculés sur chaque volet après correction
...
Les données sont séparées par saison de suivi de 2012-2013 à 2021-2022.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{equation}
  \label{eq_tau}
...
# plot, mais pas les axes 
  ...
  if (!is.null(dateinit3)){
    rect(xleft=dateinit3,ybottom=102,xright=datefin3,ytop=ytop,col=jaune_EV,border=jaune_EV,density=6)
  }
  if (!is.null(dateinit)){
...
didson du 02 octobre au 05 octobre (Figures \ref{debitturbidite} et
\ref{fonctionnementvanne}). Trois petites crues sont observées (débit supérieur à
\SI{100}{\cubic\metre\per\second}) en mi-décembre, la première quinzaine de
janvier (crue principale), puis fin février.  Les
turbidités montent au-delà de \underline{50 NTU} au cours de la crue de janvier
(Figure \ref{debitturbidite}).
...
\input{../table/2022/depouillement.tex}
Cette année les temps de dépouillement sont de 20 minutes alors qu'ils
...
\begin{figure}[htbp]
      ...
      \begin{subfigure}{0.4\textwidth}
                \centering
                \includegraphics{2022/tpsboxplot}
                \caption[Temps de dépouillement par fichier de 30
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.4\textwidth]{2022/mulet}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.4\textwidth]{2022/mulet2}
...
\input{../table/2022/tabfls.tex}
\input{../table/2022/tabflsall.tex}
...
février et avril sont visibles (en orange, Figure \ref{position10}). En
avril il s'agit essentiellement de problèmes de communication via le
...
\begin{figure}[htbp]
  ...
  la saison, cet effet est pris en compte dans le modèle mais ignoré dans la
...
présentés en Figures \ref{efficacite} et \ref{Ncor}
 (les deux figures sont en annexe).
...
\begin{figure}[htbp]
        ...
        \begin{subfigure}{0.4\textwidth}
                ...
                15mm,clip]{2022/heure_jourcomplet.png}
...
\input{../table/2022/repartition_horaire.tex}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\vspace{\dimexpr.8pt+\fboxsep\relax}
%\noindent       
%\fbox{%
...
\input{../table/2022/comportement.tex}
\input{../table/2022/traversee.tex}
...
(Nprim04all <- sum(colSums(ddde[,c("drr_eelplus","drr_eelminus")],na.rm=TRUE))) # 20898
(Nprim04allCY <- sum(colSums(ddde[ddde$dsf_season==saison,c("drr_eelplus","drr_eelminus")],na.rm=TRUE))) # 1937
sum(d3ej$Nprimo4) # plus faible car limité de jour #19840
...
toto <- latex(final,
    ...
    longtable = TRUE,
    n.rgroup=c(7, 8, 7, 6, 6, 6, 6, 6, 7, 6),
...
La saison \Sexpr{saison} se caractérise par des débits de crue assez faibles
(3 pics à plus de 100 \si{\cubic\metre\per\second}), avec le plus important pic
observé fin décembre, début janvier. La migration s'est principalement
effectuée lors de la première augmentation de débit fin décembre avec un effectif journalier dépassant 10000
anguilles. Le pic de crue de fin février n'a permis la migration que d'un nombre
...
\input{../table/2022/tabefficacite_moyenne.tex}
...
\begin{figure}[htbp]
        ...
        \includegraphics[trim=35mm 40mm 25mm
                25mm,clip, width=0.5\textwidth]{2022/periode4_phi0_dids_-7}
        \caption{
                $\Phi$=\Sexpr{ppp$"periode4_phi0_dids_-7"$Phi}, 
                $\beta$=\Sexpr{ppp$"periode4_phi0_dids_-7"$incl}, 
                $h$=\Sexpr{ppp$"periode4_phi0_dids_-7"$prof}, 
                $O_t$=\Sexpr{ppp$"periode4_phi0_dids_-7"$N}, 
                $O_t\circ$=\Sexpr{ppp$"periode4_phi0_dids_-7"$Nsuivis},
                $N'_{4o}+$=\Sexpr{ppp$"periode4_phi0_dids_-7"$plus}, 
                $N'_{4o}-$=\Sexpr{ppp$"periode4_phi0_dids_-7"$minus},
                date= \Sexpr{iconv(ppp$"periode4_phi0_dids_-7"$date,"UTF-8")}
                }
                \label{periode4}
\end{figure}
...
\begin{figure}[htbp]
        \centering
        \includegraphics[trim=35mm 40mm 25mm
                25mm,clip, width=0.5\textwidth]{2022/periode5_phis_dids_-7}
        \caption{
                $\Phi$=\Sexpr{ppp$"periode5_phis_dids_-7"$Phi}, 
                $\beta$=\Sexpr{ppp$"periode5_phis_dids_-7"$incl}, 
                $h$=\Sexpr{ppp$"periode5_phis_dids_-7"$prof}, 
                $O_t$=\Sexpr{ppp$"periode5_phis_dids_-7"$N}, 
                $O_t\circ$=\Sexpr{ppp$"periode5_phis_dids_-7"$Nsuivis},
                $N'_{4o}+$=\Sexpr{ppp$"periode5_phis_dids_-7"$plus}, 
                $N'_{4o}-$=\Sexpr{ppp$"periode5_phis_dids_-7"$minus},
                date= \Sexpr{iconv(ppp$"periode5_phis_dids_-7"$date,"UTF-8")}
                }
                \label{periode5}
\end{figure}
...
\begin{figure}[htbp]
        \centering
        \includegraphics[trim=35mm 40mm 25mm
                25mm,clip, width=0.5\textwidth]{2022/periode9_phis_didf_-7}
        \caption{
...
\begin{figure}[htbp]
        \centering
        \includegraphics[trim=35mm 40mm 25mm
                25mm,clip, width=0.5\textwidth]{2022/periode10_phif_didf_-7}
        \caption{
                $\Phi$=\Sexpr{ppp$"periode10_phif_didf_-7"$Phi}, 
                $\beta$=\Sexpr{ppp$"periode10_phif_didf_-7"$incl}, 
                $h$=\Sexpr{ppp$"periode10_phisf_didf_-7"$prof}, 
                $O_t$=\Sexpr{ppp$"periode10_phif_didf_-7"$N}, 
                $O_t\circ$=\Sexpr{ppp$"periode10_phif_didf_-7"$Nsuivis},
                $N'_{4o}+$=\Sexpr{ppp$"periode10_phif_didf_-7"$plus}, 
                $N'_{4o}-$=\Sexpr{ppp$"periode10_phif_didf_-7"$minus},
                date= \Sexpr{iconv(ppp$"periode10_phif_didf_-7"$date,"UTF-8")}
              }
              \label{periode10}
\end{figure}
...
\begin{figure}[htbp]
        \centering
        \includegraphics[trim=35mm 40mm 25mm
                25mm,clip, width=0.5\textwidth]{2022/periode11_phis_dids_-10}
        \caption{
                $\Phi$=\Sexpr{ppp$"periode11_phis_dids_-10"$Phi}, 
                $\beta$=\Sexpr{ppp$"periode11_phis_dids_-10"$incl}, 
                $h$=\Sexpr{ppp$"periode11_phifs_dids_-10"$prof}, 
                $O_t$=\Sexpr{ppp$"periode11_phis_dids_-10"$N}, 
                $O_t\circ$=\Sexpr{ppp$"periode11_phis_dids_-10"$Nsuivis},
                $N'_{4o}+$=\Sexpr{ppp$"periode11_phis_dids_-10"$plus}, 
                $N'_{4o}-$=\Sexpr{ppp$"periode11_phis_dids_-10"$minus},
                date= \Sexpr{iconv(ppp$"periode11_phis_dids_-10"$date,"UTF-8")}               
               }
                \label{periode11}
\end{figure}
...
\subsection{Tableaux}
\onecolumn
\input{../table/2022/aovefficacite.tex}
\newpage{}
\input{../table/2022/tabfinal.tex}
\newpage
\input{../table/2022/table_effectif_mois.tex}
\newpage
\input{../table/2022/tableau_modele_miss.tex}
\normalsize
\begin{landscape}
\begin{figure}
