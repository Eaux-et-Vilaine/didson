...
l'ordre de 700 $m^3.s^{-1}$. Le DIDSON est enlevé de l'eau lors du deuxième pic de crue
le 03/02. 
...
&min(1,\\
&b_0+b_1\tau+b_2\delta+b_3 k+b_4\delta:k+b_5\tau:k+b_6 saison\\
&)
\end{aligned}
\end{equation} 
...
4 jours contre 7 jours et 15 jours pour les saisons précédente
(2019-2020 et 2020-2021), et un mois pour 2018-2019 (Tableau
\ref{tabletps}). 


\input{../table/2022/depouillement.tex}
Cette année les temps de dépouillment sont de 20 minutes alors qu'ils
...
\begin{figure}[htbp]
  ...
  des mulets par mois par mois et en fonction de l'heure.}
  \label{mulet2}
\end{figure}
\begin{figure}[htbp]
  \centering
...
périodes ou le didson est mal positionné (en marron), quand c'est le cas il
...
\begin{figure}[htbp]
  ...
  didson, on distingue aisément les deux modes "mâles" centré autour de 40 cm et
  femelles centré autour de 75 cm).
...
saison (octobre novembre décembre) (Figure \ref{mosaic_taille_mois}). C'est
...
distribution homogène en janvier février. \textit{A contrario}, les anguilles de
...
\begin{figure}[htbp]
  ...
  la saison (avec un classe particulière pour les premières années ou le didson a alterné 
  son acquisition entre des distances de 2-12m et 5-15m).
...
Les sexes ratios calculés en utilisant une limite de 
 ...
 \citep{acou_estimation_2010}) s'établissent à
 \num{\Sexpr{round(100*vvv$sexe_ratio)}} \% de mâles en \Sexpr{saison}, soit la plus importante proportion de mâles observée
 jusqu'alors.
...
d'écriture disque (Figure \ref{fig_horairespb}). Pour la saison \Sexpr{saison} ce
...
limité d'anguilles argentées. Globalement, les
...
en 2019--2020 sont peut être lié à l'effet d'une grande crue
...
\begin{figure}[htbp]
        \centering
        \includegraphics[trim=35mm 40mm 25mm
                25mm,clip, width=0.5\textwidth]{2022/periode11_phis_didf_-7}
        \caption{
                $\Phi$=\Sexpr{ppp$"periode11_phis_didf_-7"$Phi}, 
                $\beta$=\Sexpr{ppp$"periode11_phis_didf_-7"$incl}, 
                $h$=\Sexpr{ppp$"periode11_phifs_didf_-7"$prof}, 
