...
\Large 
Suivi de la dévalaison d'anguilles argentées en 2024-2025 (douzième
...
\vspace{1cm}\\
\centering{\includegraphics[width=0.6\textwidth]{logo_region_bretagne.png}} 
\vspace{1cm}
\end{minipage}
}%  
...
Les dévalaisons d'anguilles argentées ont été suivies, pour la douzième année
 ...
 c'est à dire quand le didson est bien positionné par rapport aux écoulements,
  ...
  rapport (globalement tendance à la baisse).\\
  ...
  \smallskip
 Silver eel migrations were monitored, for the twelfth year
...
briand_suivi_2021, briand_suivi_2022, briand_suivi_2023, briand_suivi_2024}.
...
req@sql <- str_c("select * from did.v_dddedj where enj_date>'",toutdebutdate,
    "' and enj_date<'",datefin,"'")
d3edj <- query(req, base) %>% getquery()
#req@sql <- str_c("select * from did.v_dddedj where enj_date='","2022-03-28","'")
...
En 2024--2025, le didson et son rotateur montrent des signes de fatigue, avec de
...
plotdebitturbidite <- function(ylim = 1000, ytop=800, d3ej, d3edj){
  thisseason_d3ej <- d3ej$enj_date>=datedebut
  thisseason_d3edj <- d3edj$enj_date>=datedebut
...
# plot, mais pas les axes 
  ...
  plot(d3edj$enj_date[thisseason_d3edj],d3edj$deb_qtotalj[thisseason_d3edj],type="l",yaxt='n',col="black",ylab="",xlab="date",ylim=c(0,ylim))
  ...
  polygon(c(min(d3edj$enj_date[thisseason_d3edj])-1,
          d3edj$enj_date[thisseason_d3edj],
          max(d3edj$enj_date[thisseason_d3edj])+1),
  ...
  points(d3edj$enj_date[thisseason_d3edj][thisseason_d3edj],d3edj$debitvilainej[thisseason_d3edj],col=turquoise_EV,type="l")
  points(d3edj$enj_date[thisseason_d3edj],d3edj$enj_turb[thisseason_d3edj]*4,col="black",type="l",lty=2)
...
D0 <- D0 %>% mutate(
    ...
    day = as.numeric(strftime(dsf_timeinit,format="%j", tz ="UTC")),
...
D0 <- D0 %>% 
    ...
    mutate(ndays = 
            case_when(day<250 & !leap_year ~ day + 365,
                day<250 & leap_year ~ day + 366,
                TRUE ~day),
        ndays =  ndays - min(ndays)+1) 
...
stopifnot(D0[as.character(D0$dsf_timeinit)=="2013-01-01 01:00:00","ndays"]==103)
stopifnot(D0[as.character(D0$dsf_timeinit)=="2014-01-01 01:00:00","ndays"]==102)
stopifnot(D0[as.character(D0$dsf_timeinit)=="2017-01-01 01:00:00","ndays"]==103)
...
Dj$day[Dj$day<250 & !Dj$leap_year] <- Dj$day[Dj$day<250 & !Dj$leap_year]+365
Dj$day[Dj$day<250 & Dj$leap_year] <- Dj$day[Dj$day<250  & Dj$leap_year]+366
Dj$ndays[!Dj$leap_year] <- Dj$day[!Dj$leap_year] - min(Dj$day)+1
Dj$ndays[Dj$leap_year] <- Dj$day[Dj$leap_year] - min(Dj$day)
...
D3j <- D2 %>% 
    ...
    dplyr::summarize(N=sum(N, na.rm=TRUE),nb=dplyr::n(),vol=sum(volbarrage30), .groups="drop")
...
D5j <- left_join(D4j,Dj,by=c("ndays", "dsf_season"))
D5j <- left_join(D5j, D2j %>% select(-N), by=c("ndays", "dsf_season")) # compte les périodes fls et fond sur surface
...
#View(D5j[,c("enj_date","nb.0","vol.0","N.0","nb.1","vol.1","N.1","N")])
...
AIC(g3)
summary(g4 <- mgcv::gam(N~s(ndays, by=dsf_season, k=5, bs= "ts")+s(deb_qtotalj),dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g4)
summary(g5 <- mgcv::gam(N~dsf_season+ s(ndays, by=dsf_season, k=5, bs= "ts")+s(deb_qtotalj) ,dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g5)
summary(g6 <- mgcv::gam(N~dsf_season+s(ndays, by=dsf_season, k=5, bs= "ts")+s(deb_qtotalj)+cumflow ,dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g6)
summary(g7 <- mgcv::gam(N~dsf_season+s(ndays, by=dsf_season, k=5, bs= "ts")+s(deb_qtotalj)+cumflow + delta_1d,dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g7) # delta NS
summary(g8 <- mgcv::gam(N~dsf_season+s(ndays, by=dsf_season, k=5, bs= "ts")+s(deb_qtotalj)+cumflow+ delta_3d,dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g8) # delta NS
summary(g9 <- mgcv::gam(N~dsf_season+s(ndays, by=dsf_season, k=5, bs= "ts")+s(deb_qtotalj)+ cumflow + delta_7d,dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g9) # delta NS 
summary(g10 <- mgcv::gam(N~dsf_season+s(ndays, k=5, bs= "ts")+s(deb_qtotalj)+cumflow + s(delta_7d),dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g10) 

# J'ai du virer tous les +s(enj_turb) car je ne l'ai plus dans mon modèle
# note les cumflow a un pb de concurvity, donc je le vire du gam
bestg <- g9
...
#####################################
#D7j[D7j$pred>1e6 & !is.na(D7j$pred),c("dsf_season","enj_date","N.0","N.1","N.2","ndays")]
D7j$ndays0 <- D7j$ndays

p1 <-    is.na(D7j$N)
D7j[,"pred"]<- predict(g9,newdata=D7j,type="response")
# Limit the predictions to the max observed for this month
max_per_month_and_year <- D7j |> 
    filter(!is.na(enj_date)) |> # some are empty beginning and end of the seaon or even in 2018-2019 full days missing
...
quickg <- glm(maxmy ~dsf_season + month, data = max_per_month_and_year[!is.infinite(max_per_month_and_year$max),])
max_per_month_and_year[is.infinite(max_per_month_and_year$max),"max"] <- predict(quickg, max_per_month_and_year[is.infinite(max_per_month_and_year$max),])
...
plot(dj2$enj_date,dj2$N,type="n",ylab="N & 10debit m3.s-1",xlab="date", ylim=c(0,16000))
...
text(dj2$enj_date[1], 14000, "Did. FOND", col=rouille, adj = c(0, 0))
points(dj2$enj_date[dj2$pos_f>0 & !is.na(dj2$pos_f)], rep(13500, sum(dj2$pos_f>0, na.rm=TRUE)), col=rouille, type="p", lwd=2, pch=15)
text(dj2$enj_date[1], 12000, "Did. SURF", col="gray", adj = c(0, 0))
points(dj2$enj_date[dj2$pos_s>0& !is.na(dj2$pos_f)], rep(11500, sum(dj2$pos_s>0, na.rm=TRUE)), col="gray", type="p", lwd=2, pch=15)
...
non linéaire, le débit à 7 jours, le débit total, le cumul du débit ayant transité par le barrage 
depuis le début de la saison. La turbidité qui était utilisée jusqu'à la dernière saison est retirée du modèle car elle n'est plus collectée par l'usine d'eau. pour prédire les migrations.
 Les tendances saisonnières permettent un ajutement d'une tendance saisonnière
 globablement à l'augmentation, sauf pour 2016-2017 et 2017-2018. L'effet saison
 montre une tendance à la baisse des effectifs depuis 2012-2013, et le cumul de
 débit présente une tendance linéaire à la baisse que l'on peut interpréter de
 manière simple : plus y a eu d'eau écoulée, moins il reste d'anguilles.
 Le modèle explique \num{\Sexpr{round(100*summary(bestg)$dev.expl,2)}} \% de la
 déviance.
...
%=====================================================================  
  \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2025/fig_effectif_2023_2024}
  \caption[Effectif]{Effectifs journaliers estimés sur l'ensemble du barrage
...
%=====================================================================  
  \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2025/fig_effectif_2022_2023}
...
%=====================================================================  
  \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2025/fig_effectif_2021_2022}
...
%=====================================================================  
  \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2025/fig_effectif_2020_2021}
...
%=====================================================================  
  \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2025/fig_effectif_2019_2020}
...
%=====================================================================  
  \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2025/fig_effectif_2018_2019}
...
%=====================================================================  
  \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2025/fig_effectif_2017_2018}
...
%=====================================================================  
  \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2025/fig_effectif_2016_2017}
...
%=====================================================================  
  \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2025/fig_effectif_2015_2016}
...
%=====================================================================  
  \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2025/fig_effectif_2014_2015}
...
%=====================================================================  
  \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2025/fig_effectif_2013_2014}
...
%=====================================================================  
  \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2025/fig_effectif_2012_2013}
