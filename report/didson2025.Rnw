...
plotdebitturbidite(ylim= 700, ytop= 600, d3ej=d3ej, d3edj=d3edj)
dev.off()
@
...
AIC(g4)
summary(g5 <- mgcv::gam(N~s(ndays, by=dsf_season)+s(deb_qtotalj),dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g5)
summary(g6 <- mgcv::gam(N~dsf_season+s(ndays, by=dsf_season, k=5)+s(deb_qtotalj)+s(cumflow, k=3) ,dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g6)
summary(g7 <- mgcv::gam(N~dsf_season+s(ndays, by=dsf_season, k=5)+s(deb_qtotalj)+s(cumflow, k=3) + s(delta_1d),dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g7) # delta NS
summary(g8 <- mgcv::gam(N~dsf_season+s(ndays, by=dsf_season, k=5)+s(deb_qtotalj)+s(cumflow, k=3) + s(delta_3d),dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g8) # delta NS
summary(g9 <- mgcv::gam(N~dsf_season+s(ndays, by=dsf_season, k=5)+s(deb_qtotalj)+cumflow + s(delta_7d),dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g9) # delta NS 
...
plot(bestg,pages=1, all.terms=TRUE, shade=TRUE, ylim=c(-20,20))
dev.off()
...
toto <- latex(final,
    ...
    longtable = TRUE,
    n.rgroup=c(7, 8, 7, 6, 6, 6, 6, 6, 7, 6, 6 , 6),
