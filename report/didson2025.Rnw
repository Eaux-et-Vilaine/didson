...
D3j <- D2 %>% 
    dplyr::select(dsf_season,ndays,N,indexOK,indexOK,volbarrage30) %>%
...
# On ramène dans ce tableau, les données journalières et le c
...
# sur ce fichier on passe de 2100 lignes à 2087
...
AIC(g3)
summary(g4 <- mgcv::gam(N~s(ndays, by=dsf_season, k=5)+s(deb_qtotalj),dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g4)
summary(g5 <- mgcv::gam(N~dsf_season+ s(ndays, by=dsf_season, k=5)+s(deb_qtotalj) ,dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g5)
summary(g6 <- mgcv::gam(N~dsf_season+s(ndays, by=dsf_season, k=5)+s(deb_qtotalj)+cumflow ,dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g6)
summary(g7 <- mgcv::gam(N~dsf_season+s(ndays, by=dsf_season, k=5)+s(deb_qtotalj)+cumflow + delta_1d,dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g7) # delta NS
summary(g8 <- mgcv::gam(N~dsf_season+s(ndays, by=dsf_season, k=5)+s(deb_qtotalj)+cumflow+ delta_3d,dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g8) # delta NS
summary(g9 <- mgcv::gam(N~dsf_season+s(ndays, by=dsf_season, k=5)+s(deb_qtotalj)+ cumflow + delta_7d,dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g9) # delta NS 
summary(g10 <- mgcv::gam(N~dsf_season+s(ndays, k=5)+s(deb_qtotalj)+cumflow + s(delta_7d),dat=D7j[D7j$N>0,],family=Gamma(link="log")))
AIC(g10) 
...
# pour éviter les extrapolations quand le modèle part en sucette
D7j$ndays[D7j$dsf_season=="2019-2020" & D7j$ndays>170] <- 170
D7j$ndays[D7j$dsf_season=="2018-2019" & D7j$ndays<60 ] <- 60
p1 <-    is.na(D7j$N)
...
# Dans D4j on a pas encore multipl
...
plot(dj2$enj_date,dj2$N,type="n",ylab="N & 10debit m3.s-1",xlab="date", ylim=c(0,6000))
...
points(dj2$enj_date[dj2$pos_f>0 & !is.na(dj2$pos_f)], rep(3000, sum(dj2$pos_f>0, na.rm=TRUE)), col=rouille, type="p", lwd=2, pch=15)
text(dj2$enj_date[1], 3700, "Did. SURF", col="gray", adj = c(0, 0))
points(dj2$enj_date[dj2$pos_s>0& !is.na(dj2$pos_f)], rep(3500, sum(dj2$pos_s>0, na.rm=TRUE)), col="gray", type="p", lwd=2, pch=15)
...
saison \Sexpr{saison}, le nombre de données correspondant à ce type de configuration
...
($\odot$). L'effectif extrapolé à partir des migrations observées le même
jour est de  N $\otimes$=\num{\Sexpr{vvv$N.otimes}} anguilles argentées.

Sur l'ensemble des années, il y a eu en moyenne
\num{\Sexpr{round(mean(vvv$number.missingday.all$N))}} journées avec aucun enregistrement du fait de problèmes d'acquisition ou
d'écriture disque (Figure \ref{fig_horairespb}). Pour la saison \Sexpr{saison},
ce nombre très limité avec seulement \num{\Sexpr{vvv$number.missingday}}
journée manquante.
...
utilisé pour extrapoler les effectifs en migration à partir des valeurs des jours proches ($\oplus$).
 Ce modèle utilise une tendance saisonnière non linéaire, le débit à 7 jours,
 le débit total, le cumul du débit ayant transité par le barrage depuis le début
 de la saison. La turbidité qui était utilisée jusqu'à la dernière saison est retirée
 du modèle car elle n'est plus collectée par l'usine d'eau. pour prédire les migrations.
...
\begin{figure}[htbp]
        ...
        \includegraphics[trim=35mm 40mm 25mm
                25mm,clip, width=0.5\textwidth]{2025/periode2_phi0_didf_5}
        \caption{
                $\Phi$=\Sexpr{ppp$"periode2_phi0_didf_5"$Phi}, 
                $\beta$=\Sexpr{ppp$"periode2_phi0_didf_5"$incl}, 
                $h$=\Sexpr{ppp$"periode2_phi0_didf_5"$prof}, 
                $O_t$=\Sexpr{ppp$"periode2_phi0_didf_5"$N}, 
                $O_t\circ$=\Sexpr{ppp$"periode2_phi0_didf_5"$Nsuivis},
                $N'_{4o}+$=\Sexpr{ppp$"periode2_phi0_didf_5"$plus}, 
                $N'_{4o}-$=\Sexpr{ppp$"periode2_phi0_didf_5"$minus},
                date= \Sexpr{iconv(ppp$"periode2_phi0_didf_5"$date,"UTF-8")}
                }
                \label{periode2}
\end{figure}
...
\begin{figure}[htbp]
        ...
        \caption{
                $\Phi$=\Sexpr{ppp$"periode3_phif_didf_5"$Phi}, 
                $\beta$=\Sexpr{ppp$"periode3_phif_didf_5"$incl}, 
                $h$=\Sexpr{ppp$"periode3_phif_didf_5"$prof}, 
                $O_t$=\Sexpr{ppp$"periode3_phif_didf_5"$N}, 
                $O_t\circ$=\Sexpr{ppp$"periode3_phif_didf_5"$Nsuivis},
                $N'_{4o}+$=\Sexpr{ppp$"periode3_phif_didf_5"$plus}, 
                $N'_{4o}-$=\Sexpr{ppp$"periode3_phif_didf_5"$minus},
                date= \Sexpr{iconv(ppp$"periode3_phif_didf_5"$date,"UTF-8")}
                }
                \label{periode3}
\end{figure}
...
\begin{figure}[htbp]
        \centering
        \includegraphics[trim=35mm 40mm 25mm
                25mm,clip, width=0.5\textwidth]{2025/periode4_phif_didf_-7}
        \caption{
                $\Phi$=\Sexpr{ppp$"periode4_phif_didf_-7"$Phi}, 
                $\beta$=\Sexpr{ppp$"periode4_phif_didf_-7"$incl}, 
                $h$=\Sexpr{ppp$"periode4_phif_didf_-7"$prof}, 
                $O_t$=\Sexpr{ppp$"periode4_phif_didf_-7"$N}, 
                $O_t\circ$=\Sexpr{ppp$"periode4_phif_didf_-7"$Nsuivis},
                $N'_{4o}+$=\Sexpr{ppp$"periode4_phif_didf_-7"$plus}, 
                $N'_{4o}-$=\Sexpr{ppp$"periode4_phif_didf_-7"$minus},
                date= \Sexpr{iconv(ppp$"periode4_phif_didf_-7"$date,"UTF-8")}}
        \label{periode6}
\end{figure}
...
\begin{figure}[htbp]
        \centering
        \includegraphics[trim=35mm 40mm 25mm
                25mm,clip, width=0.5\textwidth]{2025/periode6_phis_didf_-7.png}
        \caption{
                $\Phi$=\Sexpr{ppp$"periode6_phis_didf_-7.png"$Phi}, 
                $\beta$=\Sexpr{ppp$"periode6_phis_didf_-7.png"$incl}, 
                $h$=\Sexpr{ppp$"periode6_phis_didf_-7.png"$prof}, 
                $O_t$=\Sexpr{ppp$"periode6_phis_didf_-7.png"$N}, 
                $O_t\circ$=\Sexpr{ppp$"periode6_phis_didf_-7.png"$Nsuivis},
                $N'_{4o}+$=\Sexpr{ppp$"periode6_phis_didf_-7.png"$plus}, 
                $N'_{4o}-$=\Sexpr{ppp$"periode6_phis_didf_-7.png"$minus},
                date= \Sexpr{iconv(ppp$"periode6_phis_didf_-7.png"$date,"UTF-8")}               
               }
                \label{periode5}
\end{figure}
...
\begin{figure}[htbp]
        \centering
        \includegraphics[trim=35mm 40mm 25mm
                25mm,clip, width=0.5\textwidth]{2025/periode7_phif_didf_6}
        \caption{
                $\Phi$=\Sexpr{ppp$"periode7_phif_didf_6"$Phi}, 
                $\beta$=\Sexpr{ppp$"periode7_phif_didf_6"$incl}, 
                $h$=\Sexpr{ppp$"periode7_phif_didf_6"$prof}, 
                $O_t$=\Sexpr{ppp$"periode7_phif_didf_6"$N}, 
                $O_t\circ$=\Sexpr{ppp$"periode7_phif_didf_6"$Nsuivis},
                $N'_{4o}+$=\Sexpr{ppp$"periode7_phif_didf_6"$plus}, 
                $N'_{4o}-$=\Sexpr{ppp$"periode7_phif_didf_6"$minus},
                date= \Sexpr{iconv(ppp$"periode7_phif_didf_6"$date,"UTF-8")}               
               }
                \label{periode7}
\end{figure}
...
\begin{figure}[htbp]
        \centering
        \includegraphics[trim=35mm 40mm 25mm
                25mm,clip, width=0.5\textwidth]{2025/periode8_phi0_didf_6}
        \caption{
                $\Phi$=\Sexpr{ppp$"periode8_phi0_didf_6"$Phi}, 
                $\beta$=\Sexpr{ppp$"periode8_phi0_didf_6"$incl}, 
                $h$=\Sexpr{ppp$"periode8_phi0_didf_6"$prof}, 
                $O_t$=\Sexpr{ppp$"periode8_phi0_didf_6"$N}, 
                $O_t\circ$=\Sexpr{ppp$"periode8_phi0_didf_6"$Nsuivis},
                $N'_{4o}+$=\Sexpr{ppp$"periode8_phi0_didf_6"$plus}, 
                $N'_{4o}-$=\Sexpr{ppp$"periode8_phi0_didf_6"$minus},
                date= \Sexpr{iconv(ppp$"periode8_phi0_didf_6"$date,"UTF-8")}               
               }
                \label{periode8}
\end{figure}
