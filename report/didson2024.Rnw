...
drawdi<-function(
  ...
  poly2<-	cbind(c(18,18,retrait,retrait,18),c(fond,fond+hpas,fond+hpas,fond, fond))
  if (volet) poly2<-	cbind(c(
            18,
            18,
            retrait,
            retrait,
  ...
  if (graph){
    ...
    if (is.infinite(perpslope)){
     abline(v=c(d1,d2),lty=3, col = "white")
    } else {
    abline(b=perpslope,a=fond-perpslope*dist,lty=3, col="white") # perpendiculaire au point ou le centre touche le fond
    }
  ...
  return(list("area_intersect"=round(st_area(st_intersection(detection_frame,migration_frame)),2),
