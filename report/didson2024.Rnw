...
sexe_ratio <- C2 %>%  
    mutate(sexe = ifelse(taille<50,'male','femelle')) %>%    
    group_by(saison, sexe) %>%
    summarize(N= sum(N)) %>%
    pivot_wider(id_cols=saison, names_from=sexe, values_from=N) %>%
    mutate(sex_ratio = male/(male + femelle))
vvv$sexe_ratio <- pull(sexe_ratio[sexe_ratio$saison==saison, "sex_ratio"])

options(encoding = "UTF-8")
xtsexeratio <- xtable::xtable(sexe_ratio,
    label="tabsexeratio",
    caption=iconv("Sexe ratio (pourcentage de mâles) dans les anguilles observées après correction de l'efficacité."),
    digits=c(0,0,0,0,2)
)
print(xtsexeratio, file=str_c(tabwdy,"tabsexeratio.tex"),
    table.placement="htbp",
    caption.placement = "top",
    NA.string = "",
...
Les sexes ratios calculés en utilisant une limite de 
 ...
 \num{\Sexpr{round(100*vvv$sexe_ratio)}} \% de mâles en \Sexpr{saison}.
 Les chiffres de sexe ratio montrent une diminution puis une réaugmentation de
 la proportion de mâles (Table \ref{tabsexeratio}).
\input{../table/2024/tabsexeratio.tex}
...
(Nprim04all <- sum(colSums(ddde[,c("drr_eelplus","drr_eelminus")],na.rm=TRUE))) # 22373
(Nprim04allCY <- sum(colSums(ddde[ddde$dsf_season==saison,c("drr_eelplus","drr_eelminus")],na.rm=TRUE))) # 1116
...
[TODO CHECK ICI APPEL A e3ejok QUI EST SEULEMENT DANS LES ANCIENS CALCULS, 
DEPLACER CHUNK NOMBRE ALL AVANT ET FAIRE REFERENCE A CELUI CI ]
Un total de
...
<<load_all_years, echo=FALSE, eval=FALSE, results=hide>>=
# d3ej vient de ddde mais limité de jour
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2024/horairespb2024}
