...
\vspace{1cm}\\
\centering{\includegraphics[width=\textwidth]{logo_region_bretagne.png}} 
\end{minipage}
}%  
\vfill
\hfill{ %
...
avec des débits de l'ordre de 700 $m^3.s^{-1}$. Le didson a été enlevé
...
380 $m^3.s^{-1}$. Après une période de débit plus faible, une deuxième
crue à partir du 31 décembre a ramené des débits de l'ordre de 350 $m^3.s^{-1}$. 
...
mi-avril, avec 2 pics à 300 $m^3.s^{-1}$ début novembre et mi-décembre,
un gros pic à 650 $m^3.s^{-1}$ début janvier suivi d'un autre à 450 $m^3.s^{-1}$ mi-janvier,
puis de 3 crues de 350 à 400 $m^3.s^{-1}$ de février à la mi-avril.
...
plotdebitturbidite <- function(ylim = 1000, ytop=800){
...
# plot, mais pas les axes 
  ...
  plot(d3edj$enj_date[thisseason],d3edj$deb_qtotalj[thisseason],type="l",yaxt='n',col="black",ylab="",xlab="date",ylim=c(0,ylim))
  ...
  polygon(c(min(d3edj$enj_date[thisseason])-1,d3edj$enj_date[thisseason],max(d3edj$enj_date[thisseason])+1),c(100,pmax(100,d3edj$deb_qtotalj[thisseason]),100),col="grey",border=NA)
...
sept grosses crues sont observées (débit supérieur à
