...
req@sql <- str_c("select * from did.t_didsonfiles_dsf  dsf 	
        ...
        where dsr_csotismin and dsf_timeinit>'",toutdebut,
    "' and dsf_timeinit<'",fin,
    "'") # dsf_dsr_drr_psf    
...
d3ej <-
    subset(ddde, (ddde$hourhour > 17 |
              ddde$hourhour <= 9) & !is.na(ddde$date))  # ddde de jour
d3ej[d3ej$dsf_timeinit>'2022-12-21 04:00:00' & d3ej$dsf_timeinit<'2022-12-21 10:00:00',]
ddde[ddde$dsf_id==225363,"hour"]
...
plotdebitturbidite(ylim= 400, ytop= 350)
dev.off()
@
...
D2$N4[D2$vol4==0] <- NA
# test
sum(D2$N[D2$indexOK==1], na.rm=TRUE) #889654.6
sum(D2$N4[D2$indexOK==1]) 
